 ...
 */
@ApiAudience.Private
public class KijiNSNBulkImport extends JobTool<KijiBulkImportJobBuilder> {
  private static final Logger LOG = LoggerFactory.getLogger(KijiNSNBulkImport.class);
  ...
  public String getName() {
    return "nsn-bulk-import";
  }

  /** {@inheritDoc} */
  @Override
  public String getDescription() {
    return "Bulk import metadata into Repository table";
  }
  ...
  protected void validateFlags() throws Exception {
    ...
    Preconditions.checkArgument(mrJobOutput instanceof KijiTableMapReduceJobOutput,
        "NSN-bulk-import jobs output format must be 'hfile' or 'kiji', but got output spec '%s'.",
        mOutputFlag);
  ...
  protected void configure(KijiBulkImportJobBuilder jobBuilder)
    ...
    if (!(output instanceof KijiTableMapReduceJobOutput)) {
      throw new RuntimeException(String.format(
          "Invalid nsn-bulk-importer job output: '%s'; must output to a Kiji table.", mOutputFlag));
    }
  ...
  protected int run(List<String> nonFlagArgs) throws Exception {
    ...
    if (mOutput instanceof DirectKijiTableMapReduceJobOutput) {
      if (jobStatus == 0) {
        LOG.info("NSN-bulk-import job for table {} completed successfully.",
            mOutput.getOutputTableURI());
      } else {
        LOG.error("NSN-bulk-import job failed, output table {} may have partial writes.",
            mOutput.getOutputTableURI());
      }
    } else if (mOutput instanceof HFileMapReduceJobOutput) {
      if (jobStatus == 0) {
        // Provide instructions for completing the bulk import.
        LOG.info("NSN-bulk-import job completed successfully. "
  ...
  public static void main(String[] args) throws Exception {
    System.exit(new KijiToolLauncher().run(new KijiNSNBulkImport(), args));
  }
}
